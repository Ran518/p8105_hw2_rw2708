...
Description: 1) For the first dataset (wheel_data), there exist `r nrow(wheel_data)` observations, with `r ncol(wheel_data)` variables, and it's key variables are `r colnames(wheel_data)[1:6]`.  Furthermore,the median number of sports balls in a dumpster in 2017 is `r median(pull(wheel_data,sports_balls))`. 2) For the second dataset (precipitation2018_data), it contains the precipitation data in 2018. There exist `r nrow(precipitation2018_data)` observations, with `r ncol(precipitation2018_data)` variables, and it's key variables are `r ls(precipitation2018_data)`. Moreover, the total precipitation in 2018 is `r sum(pull(precipitation2018_data,total))`. 3) For the third dataset (precipitation2017_data), it contains the precipitation data in 2017. There exist `r nrow(precipitation2017_data)` observations, with `r ncol(precipitation2017_data)` variables, and it's key variables are `r ls(precipitation2017_data)`. 4) As for the last dataset (combine_data), it contains the precipitation data in 2017 and 2018. There exist `r nrow(combine_data)` observations, with `r ncol(combine_data)` variables, and it's key variables are `r ls(combine_data)`. 
